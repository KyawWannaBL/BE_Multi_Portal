...
echo "🚀 Fixing src/lib/portalRegistry.ts and src/lib/recentNav.ts exports..."
...
// Fallback exports for older files that might be calling these
export function pushRecent(path: string) {
  addRecentNav({ path, label_en: path, label_mm: path });
}

export function getRecent() {
  return getRecentNav().map((r) => ({ path: r.path, at: r.timestamp }));
}
