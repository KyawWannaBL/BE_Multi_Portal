...
# ==========================================================
# EN: Fix Role/RLS mismatch + add courier_locations backend (Realtime) + duplicate detectors
# MY: Role/RLS mismatch ပြင် + courier_locations backend (Realtime) ထည့် + duplicate detectors
# ==========================================================

if [ ! -f "package.json" ]; then
  echo "❌ EN: Run this from repo root (package.json not found)."
  echo "❌ MY: package.json ရှိတဲ့ repo root မှ run လုပ်ပါ။"
  exit 1
fi

STAMP="$(date +%Y%m%d_%H%M%S)"
BK=".backup_critical_${STAMP}"
mkdir -p "$BK"
...
backup_file "supabase/migrations/20260306_010_critical_fixes_role_realtime.sql"
backup_file "supabase/functions/set-role-claim/index.ts"

mkdir -p supabase/migrations scripts/sql

# ==========================================================
# A) SQL Migration
# ==========================================================
write_file "supabase/migrations/20260306_010_critical_fixes_role_realtime.sql" <<'SQL'
begin;
...
cp -f "supabase/migrations/20260306_010_critical_fixes_role_realtime.sql" "scripts/sql/critical_fixes_role_realtime.sql"

echo "✅ EN: Migration written: supabase/migrations/20260306_010_critical_fixes_role_realtime.sql"
echo "✅ MY: Migration ရေးပြီး: supabase/migrations/20260306_010_critical_fixes_role_realtime.sql"

# ==========================================================
# B) Patch Edge Function set-role-claim (write BOTH app_metadata.role + app_metadata.app_role)
# ==========================================================
if [ -f "supabase/functions/set-role-claim/index.ts" ]; then
  echo "== EN: Patching set-role-claim to write role + app_role | MY: role/app_role နှစ်ခုလုံးရေးမယ် =="

  cat > supabase/functions/set-role-claim/index.ts <<'TS'
...
TS
  ...
  echo "✅ EN: set-role-claim patched."
  echo "✅ MY: set-role-claim ပြင်ပြီးပါပြီ။"
else
  echo "⚠️ EN: supabase/functions/set-role-claim/index.ts not found. Skipping patch."
  echo "⚠️ MY: set-role-claim မတွေ့လို့ skip လုပ်ပါမယ်။"
fi
...
echo ""
echo "NEXT STEPS:"
echo "EN: 1) Apply DB migration: supabase db push   (or run scripts/sql/critical_fixes_role_realtime.sql)"
echo "MY: 1) DB migration run: supabase db push (သို့) scripts/sql/critical_fixes_role_realtime.sql ကို run"
echo "EN: 2) Deploy edge function (if exists): supabase functions deploy set-role-claim"
echo "MY: 2) edge function deploy: supabase functions deploy set-role-claim"
echo "EN: 3) IMPORTANT: Users must re-login to refresh JWT claims."
echo "MY: 3) အရေးကြီး: JWT refresh ဖြစ်အောင် user များ logout/login ပြန်လုပ်ပါ။"
echo "EN: 4) Check duplicates: select * from public.dup_public_users_email_v;  select * from public.dup_profiles_employee_id_v;"
echo "MY: 4) duplicates စစ်: select * from public.dup_public_users_email_v;  select * from public.dup_profiles_employee_id_v;"
echo "=========================================================="