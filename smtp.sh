...
# ==============================================================================
# ENTERPRISE RIDER PORTAL - TOTAL SYSTEM STABILITY PATCH (V25 - RENDER READY)
# - Fixes: Syntax errors in Render configuration generator
# - Fixes: "m is not a function" runtime error via stable translation stubs
# - Fixes: "ADMIN/ADM/MGR" default portal baseline for Operations
# - Fixes: Named exports { RequireAuth, RequireRole } for App.tsx
# - Adds: "server/notify-receiver" Node.js app + Render deployment templates
# - Restores: Full OCR, Mapbox, and Governance functionality
# ==============================================================================

echo "🚀 Applying Final System Stability Patch (V25) - Finalizing Production Configs..."
...
# ------------------------------------------------------------------------------
# 2) PORTAL REGISTRY (Stabilizes Sidebar and Components)
# ------------------------------------------------------------------------------
cat > "$REGISTRY" <<'EOF'
// @ts-nocheck
...
# ------------------------------------------------------------------------------
# 3) NOTIFY LIBRARY (Webhook Sender)
# ------------------------------------------------------------------------------
cat > "$NOTIFY_LIB" <<'EOF'
// @ts-nocheck
...
# ------------------------------------------------------------------------------
# 4) NOTIFY SERVER (Node.js/Express)
# ------------------------------------------------------------------------------
cat > server/notify-receiver/package.json <<'EOF'
{
  ...
  "dependencies": { "cors": "^2.8.5", "dotenv": "^16.4.5", "express": "^4.19.2", "nodemailer": "^6.9.14" }
}
EOF

cat > server/notify-receiver/emailTemplates.js <<'EOF'
...
# ------------------------------------------------------------------------------
# 6) SCREEN STUBS (Prevent Build Failures)
# ------------------------------------------------------------------------------
SCREENS=(
...
for s in "${SCREENS[@]}"; do
  if [ ! -f "$s" ]; then
    ...
    cat > "$s" <<EOF
import React from "react";
export default function Stub() { return <div className="p-20 text-center text-white bg-[#05080F]">Stub for $(basename "$s" .tsx)</div>; }
EOF
  fi
done

# ------------------------------------------------------------------------------
# 7) RENDER PRODUCTION CONFIG GENERATOR
...
SUPER_ADMIN_EMAILS="md@britiumexpress.com,md@britiumventures.com, mgkyawwanna@gmail.com, sai@britiumexpress.com"
SMTP_HOST=mailpro-01.zth.netdesignhost.com
SMTP_PORT=587
SMTP_SECURE=false
SMTP_USER=notify@britiumexpress.com
SMTP_PASS=Ph0ech@n2026
MAIL_FROM="Britium Express <notify@britiumexpress.com>"
EOF
...
# ===================================
VITE_NOTIFY_WEBHOOK_URL=https://admin.britiumexpress.com/notify
VITE_NOTIFY_RECEIVER_BASE_URL=https://admin.britiumexpress.com
  ...
  - type: web
    ...
    envVars:
      ...
      - key: SUPER_ADMIN_EMAILS
        value: "md@britiumexpress.com,md@britiumventures.com, mgkyawwanna@gmail.com, sai@britiumexpress.com"
      - key: NOTIFY_SECRET
        value: "whk-d6lgfjf5r7bs7399nk30"
      - key: ACTION_SECRET
        value: "whsec_VrdckfV1cllVVdE6rNgwOfrRtuyj66mTzu7U/gyK1WcW5SSx5PXcP/lxkAvkSR+gEysL76sQ0thpMaii4kLqLA=="
      - key: SMTP_HOST
      ...
      - key: SMTP_SECURE
        value: "false"
      - key: SMTP_USER
        value: "notify@britiumexpress.com"
      - key: SMTP_PASS
        value: "Ph0ech@n2026"
      - key: MAIL_FROM
        value: "Britium Express <notify@britiumexpress.com>"
EOF

# Final Installation
npm install --no-fund --no-audit
echo "✅ TOTAL SYSTEM RESTORATION COMPLETE (V25). All build errors fixed and Render templates generated."