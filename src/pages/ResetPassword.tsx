...
function readEnvHints(): string {
  return [
    ...
    "  VITE_SUPABASE_PROJECT_URL=https://dltavabvjwocknkyvwgz.supabase.co",
...
export default function ResetPassword() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // Legacy flow: #access_token=...&refresh_token=... (Implicit)
        ...
        setErrorMsg(getErrorMessage(error) || t("Invalid or expired recovery link.", "Recovery link မမှန် သို့မဟုတ် သက်တမ်းကုန်နေပါသည်။"));
        setLoading(false);
      }
    })();
  }, [t]);
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      // Best-effort: clear password-change flag if it exists in the profiles table
      try {
        ...
        // Ignore profile update errors if the main auth update succeeded
      }
