...
// EN: Convert unknown errors into readable text
// MM: မသိသော error ကိုဖတ်လို့ရသော message ပြောင်းပေးသည်
...
// EN: Check whether Supabase env is available
// MM: Supabase env ရှိ/မရှိ စစ်သည်
function supabaseReady(): boolean {
  return Boolean(SUPABASE_CONFIGURED);
}

// EN: Safe placeholder hints only
// MM: တကယ့် secret key မထည့်ဘဲ placeholder only ပြမည်
...
export default function ForcePasswordReset() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        // EN: PKCE flow from email link (?code=...)
        // MM: Email link မှ PKCE flow (?code=...) ကို handle လုပ်သည်
        ...
        // EN: Legacy hash flow (#access_token / #refresh_token)
        // MM: အဟောင်း hash token flow ကိုလည်း support လုပ်သည်
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!supabaseReady()) {
      ...
      setErrorMsg(
        t(
          "System configuration is missing (Supabase env).",
          "System config မပြည့်စုံပါ (Supabase env)."
        )
      );
      return;
    }
    ...
    try {
      ...
      // EN: Best-effort cleanup of profile password flags
      // MM: profile password flag များကို best-effort ဖြင့် reset လုပ်သည်
      try {
      ...
      setSuccessMsg(
        t(
          ...
          "စကားဝှက်ပြောင်းပြီးပါပြီ။ Login ပြန်ဝင်ပါ။"
        )
      );
        ...
        <div className="w-full max-w-md space-y-6">
          <div className="text-center space-y-2">
            ...
            <p className="text-sm text-slate-300">
              {t("Reset password", "စကားဝှက် ပြန်လည်သတ်မှတ်")}
            </p>
