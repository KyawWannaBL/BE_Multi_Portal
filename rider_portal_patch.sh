...
# ==============================================================================
# ENTERPRISE RIDER PORTAL - TOTAL SYSTEM STABILITY PATCH (V3 - BUILD FIX)
# - Fixes: "isSupabaseConfigured" export in src/lib/supabase.ts
# - Fixes: "flatByPath" export in src/lib/portalRegistry.ts
# - Fixes: "pushRecent" compatibility alias in src/lib/recentNav.ts
# - Restores: OCR + Mapbox + Batch Scanning + UI Stability
# ==============================================================================

echo "🚀 Applying Total System Stability Patch with Build Fixes..."
...
# ------------------------------------------------------------------------------
# 2) GLOBAL REGISTRY FIX (Fixes flatByPath & normalizeRole)
# ------------------------------------------------------------------------------
cat > "$REGISTRY" <<'EOF'
// @ts-nocheck
...
export const portalCountAll = () => 0;
export const portalCountForRole = () => 0;

// ✅ Build Fix: PortalSidebar.tsx expects this export
...
# ------------------------------------------------------------------------------
# 7) ROUTE INJECTION (Robust JS implementation)
# ------------------------------------------------------------------------------
node - <<'NODE'
const fs = require('fs');
...
if (fs.existsSync(path)) {
    ...
    if (!content.includes('ExecutionParcelIntakePage')) {
        // Safe check before replacing to avoid double-imports
