...
# ==============================================================================
# ENTERPRISE RIDER PORTAL - STATE OF THE ART PATCH
# - Parcel Intake: photo quality gate + OCR + barcode recognition
# - Delivery list: bilingual table + XLSX export
# - Route planning: Mapbox geocode + directions + map visualization
# - Execution portal: batch scan + auto pickup + OTP for all payments (optional) + server OTP verify
#
# Required env (frontend):
#   VITE_MAPBOX_ACCESS_TOKEN=pk.eyJ1IjoiYnJpdGl1bXZlbnR1cmVzIiwiYSI6ImNtbHVydDRwbTAwZjczZnMxbDgyODJxbHUifQ.HwgFGIQzepHOhImZLM4Knw
#
# OTP policy env (frontend):
#   VITE_OTP_VALIDATE_MODE=server|device        (default device)
#   VITE_OTP_FAIL_OPEN=true|false              (default true)
#   VITE_OTP_REQUIRE_FOR_ALL=true|false        (default false)
#
# Supabase RPC (recommended for server OTP):
#   verify_cod_otp(p_shipment_id, p_otp) -> boolean OR {valid:boolean}
# ==============================================================================
...
# ------------------------------------------------------------------------------
# 0) Dependencies (OCR + barcode + XLSX)
# ------------------------------------------------------------------------------
node - <<'NODE'
const fs = require("fs");
...
# ------------------------------------------------------------------------------
# 10) App.tsx: add route /portal/execution/intake (assumes your app already has other routes)
# Best-effort append: if your App.tsx is different, paste this route manually.
# ------------------------------------------------------------------------------
if [[ -f "$APP" ]]; then
  # minimal safe append if not already present
  if ! grep -q "/portal/execution/intake" "$APP"; then
    echo "⚠️ App.tsx exists but may differ. Add this route manually if needed:"
    echo '  <Route path="/portal/execution/intake" element={<RequireRole allow={["RIDER","DRIVER","HELPER","SUPER_ADMIN","SYS","APP_OWNER"]}><ExecutionParcelIntakePage /></RequireRole>} />'
  fi
else
  echo "⚠️ src/App.tsx not found; add route manually."
fi
...
echo
echo "Commit:"
echo "  git commit -m \"feat(execution): parcel intake OCR quality gate + route planning + otp all + batch auto pickup\""