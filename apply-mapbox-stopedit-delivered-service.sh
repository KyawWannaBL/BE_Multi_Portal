...
# EN: Add Stop inline edit (label + WayID) + Lock toggle + final geofence uses markDelivered() business logic first
# MY: Stop inline edit (label + WayID) + Lock toggle + final geofence မှာ markDelivered() business logic ကိုပထမဆုံး အသုံးပြုမည်
...
BK=".backup_mapbox_stopedit_${STAMP}"
mkdir -p "$BK"
...
echo ""
echo "✅ EN: Installed Stop editor + WayID lock + final geofence uses markDelivered(shipmentId) first."
echo "✅ MY: Stop editor + WayID lock + final geofence မှာ markDelivered(shipmentId) ကိုပထမဆုံး အသုံးပြုသည်။"
echo ""
echo "Next:"
echo "  npm run build"
